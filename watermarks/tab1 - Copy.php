...
//////////////////////////////////////////////////////WATERMARK LOGO IN PROGRESS
    ...
    function getImageAt(x, y) {
        for (let i = 0; i < images.length; i++) {
            ...
            if (x >= image.x / zoomLevel && x <= (image.x + image.img.width) / zoomLevel &&
                y >= image.y / zoomLevel && y <= (image.y + image.img.height) / zoomLevel) {
                return i;
            }
        }
        return null;
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (selectedIndex !== null) {
            isDragging = true;
            offsetX = mouseX - (images[selectedIndex].x / zoomLevel);
            offsetY = mouseY - (images[selectedIndex].y / zoomLevel);
    ...
    canvas.addEventListener('mousemove', (e) => {
        if (isDragging && selectedIndex !== null) {
            ...
            const mouseX = (e.clientX - rect.left) / zoomLevel;
            const mouseY = (e.clientY - rect.top) / zoomLevel;
            images[selectedIndex].x = (mouseX - offsetX) * zoomLevel;
            images[selectedIndex].y = (mouseY - offsetY) * zoomLevel;
            drawImages();
        }
    });
